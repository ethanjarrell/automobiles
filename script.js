...
class factory {
  constructor(make, location, airbags, abs, warranty, massbuild, customerbuild){
    ...
    this.airbags = true;
    this.abs = true;
    this.warrarnty = "60,000 miles / 3 years";
    // massbuild = function(quantity, options){
    //  return ("building" +" "+ quantity +" "+ color +" "+ trim +" "+ model +"'s");
    // }
    // customerBuild = function(color, options){
...
class sport extends car{
  constructor(model, trim, transmission, top, color, seatstrim, audio, wheelstrim, moonroof, enginetype, convertible, doors, warranty){
    super({warranty});
    this.model = model;
    this.trim = trim;
    this.transmission = transmission;
...
function massBuild(quantity, mazda3){
 return ("building" +" "+ quantity +" "+ mazda3.color +" "+ mazda3.trim +" "+ mazda3.model +"'s");
 };

 console.log(massBuild(3500, mazda3));
...
function customerBuild(color, options){
  return ("building one" +" "+ color +" "+ mazda3.trim +" "+ mazda3.model +" "+ "with the following options"+ options)
}

console.log(customerBuild( "yellow", (" : weather package, satellite radio, rear spoiler")));
...
function massBuild(quantity, MiataRF){
 return ("building" +" "+ quantity +" "+ MiataRF.color +" "+ MiataRF.trim +" "+ MiataRF.model +"'s");
 };

console.log(massBuild(15000, MiataRF));
